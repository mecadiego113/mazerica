...
{
	[CustomEditor(typeof(MazeEngine))]
	public class MazeEngineInspector: Editor 
	{
		...
		public override void OnInspectorGUI()
		{
			...
			mazeEngine.setMazePiecePack((MazePiecePack)EditorGUILayout.ObjectField("Maze Piece Pack", mazeEngine.getMazePiecePack(), typeof(MazePiecePack), true));
			...
			{
				mazeEngine.mazeGeneratedEvent = new MazeEngine.QMazeGeneratedEvent();
				mazeGeneratedEvent = serializedObject.FindProperty("mazeGeneratedEvent");
			}
			if (mazePieceGeneratedEvent == null) 
			{
				mazeEngine.mazePieceGeneratedEvent = new MazeEngine.QMazePieceGeneratedEvent();
				mazePieceGeneratedEvent = serializedObject.FindProperty("mazePieceGeneratedEvent");
			}
			if (mazeGenerateProgressEvent == null) 
			{
				mazeEngine.mazeGenerateProgressEvent = new MazeEngine.QMazeGenerateProgressEvent();
				mazeGenerateProgressEvent = serializedObject.FindProperty("mazeGenerateProgressEvent");
			}
		...
		private bool showVector2IntDirList(List<MazeVector2IntDir> list, bool foldout)
		{
			int count = EditorGUILayout.IntField("Count", list.Count);
			if (count < 0) count = 0;
			if (count != list.Count)
			{
				while (count > list.Count) list.Add(new MazeVector2IntDir(0, 0, MazeOutputDirection.NotSpecified));				
			...
			if (count > 0)
			{
				...
				if (foldout)
				{
					...
					{
						MazeVector2IntDir element = list[i];
						...
						element.direction = (MazeOutputDirection)EditorGUILayout.EnumPopup("Direction", element.direction);
		...
		private bool showVector2IntList(List<MazeVector2Int> list, bool foldout)
		{
			int count = EditorGUILayout.IntField("Count", list.Count);
			if (count < 0) count = 0;
			if (count != list.Count)
